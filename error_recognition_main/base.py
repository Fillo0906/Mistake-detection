...
from core.models.blocks import fetch_input_dim, MLP, RNN
...
def fetch_model_name(config):
    if config.task_name == const.ERROR_CATEGORY_RECOGNITION:
        ...
        if config.model_name is None:
            if config.backbone in [const.RESNET3D, const.X3D, const.SLOWFAST, const.OMNIVORE, const.PERCEPTION]:

...
class CaptainCookStepDataset(Dataset):
    ...
    def __init__(self, config, phase, split):
        ...
        with open('/content/drive/MyDrive/PoliTo/Project/error_recognition-main/annotations/step_annotations.json', 'r') as f:
            self._annotations = json.load(f)

        with open('/content/drive/MyDrive/PoliTo/Project/error_recognition-main/annotations/error_annotations.json', 'r') as f:
    ...
    def _build_error_category_labels(self):
        ...
        #return self._recording_step_error_labels
    ...
    def _build_task_specific_features_labels(self, step_features, step_has_errors, step_error_category_labels):
        ...
        if self._config.task_name == const.ERROR_RECOGNITION:
            ...
            #comment until return for it to work for 0 and 1
            ## 2) Multi-label category vector (e.g., 5 error types)
            #category_labels = sorted([l for l in self._error_category_label_name_map.keys()])  # [2,3,4,5,6]
            #C = len(category_labels)

            #label_to_index = {label: idx for idx, label in enumerate(category_labels)}
            #category_multi_hot = torch.zeros(N, C)

            # Fill multi-hot vector if categories are provided
            #if step_error_category_labels is not None:
            #    for lbl in step_error_category_labels:
            #        if lbl in label_to_index:
            #            idx = label_to_index[lbl]
            #            category_multi_hot[:, idx] = 1.0
...
def collate_fn(batch):
    ...
    #print(step_features, step_labels)
    return step_features, step_labels